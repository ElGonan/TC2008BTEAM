...
public void OnReceiveData(string result)
{
    ...
    foreach (string robotData in robotDataArray)
    {
        ...
        // Debug de parts para ver si se ha hecho bien el split
        ...
        Debug.Log("ID: " + id);  // Debug to check id
        Debug.Log("Coords: " + coords);  // Debug to check coords   
        
        if (xy.Length != 2) continue;

        if (float.TryParse(xy[0], out float x) && float.TryParse(xy[1], out float y))
        {
            Debug.Log("X: " + x + ", Y: " + y);  // Debug to check x and y
            ...
            robot.transform.position = new Vector3(x, y, 0);
       }
    }
}
